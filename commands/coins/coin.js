...
export default (class ReplyCommand extends Command {
    ...
    async updateCoins(id, toadd) {
        try {
            ...
            await fauna.query(q.Create(q.Collection('coins'), { data: { userId: id, coins: toadd } }));
            return toadd;
        }
        if (coins) {
            await fauna.query(q.Update(q.Ref(coins.ref), { data: { userId: id, coins: coins.data.coins + toadd } }));
