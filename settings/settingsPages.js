...
function update(targetObject, obj) {
    Object.keys(obj).forEach(function (key) {
        ...
        // property value is object, so recurse
            ...
            if (!('object' === typeof targetObject[key]
...
async function getSettings(id, client) {
    let settings;
    try {
        settings = await client.query(q.Get(q.Match(q.Index("guildIndex"), id)));
    }
    catch (error) {
        const settings = await client.query(q.Create(q.Collection('guildSettings'), { data: { id: id, background: "https://discordjs.guide/assets/img/8CQvVRV.cced9193.png", welcomeChannel: 0 } }));
        return settings;
    }
    return settings;
}
async function writeSettings(ref, obj, client) {
    await client.query(q.Update(q.Ref(ref), { data: obj }));
    return true;
}
...
pages[0] = class GuildSettings {
    ...
    async init() {
        ...
        await this.parent.message.react('#⃣');
        await this.parent.message.react('🌃');
    ...
    async setWelcomeChannel() {
        ...
        collector.on('collect', m => {
            ...
            m.delete();
            queryMessage.delete();
            const channel = m.content.substring(m.content.lastIndexOf("<#") + 2, m.content.lastIndexOf(">"));
